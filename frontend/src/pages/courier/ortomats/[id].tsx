...
export default function CourierOrtomatDetailPage() {
  ...
  const openCellMutation = useMutation({
    ...
    onSuccess: (data) => {
      setIsOpening(false);
      alert(`${t('courier.opening')}\n\n${data.note || ''}\n\n${t('courier.placeProduct')}: ${data.product?.name}\n${t('courier.closeCell')}`);
  ...
  const filledCells = inventory?.filter((c: Cell) => !c.isAvailable && c.productId).length || 0;
  const emptyCells = inventory?.filter((c: Cell) => c.isAvailable && c.productId).length || 0;
  const unassignedCells = inventory?.filter((c: Cell) => !c.productId).length || 0;
  const fillPercentage = Math.round((filledCells / (ortomat?.totalCells || 37)) * 100);
    ...
    <div>
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          {/* Inventory Grid */}
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-xl font-bold mb-4">{t('courier.inventory')}</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {Array.from({ length: ortomat?.totalCells || 37 }, (_, i) => i + 1).map(
                (cellNum) => {
                  const cell = inventory?.find((c: Cell) => c.number === cellNum);
                  ...
                  return (
                    <button
                      key={cellNum}
                      onClick={() => isEmpty ? handleCellClick(cell) : null}
                      disabled={!isEmpty}
                      className={`p-4 rounded-lg border-2 transition-all ${
                        isEmpty
                          ? 'border-red-500 bg-red-50 hover:bg-red-100 cursor-pointer hover:shadow-lg'
      ...
      {showModal && selectedCell && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <h2 className="text-2xl font-bold mb-4">
              {t('courier.cellNumber', { number: selectedCell.number })}
            </h2>
