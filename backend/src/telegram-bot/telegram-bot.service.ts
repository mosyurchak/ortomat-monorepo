...
export class TelegramBotService implements OnModuleInit, OnModuleDestroy {
  ...
  private setupCommands() {
    ...
    this.bot.onText(/\/start/, async (msg) => {
      ...
      await this.bot.sendMessage(
        chatId,
        `👋 Вітаю в боті "Ортомат"!\n\n` +
        `Цей бот допоможе Вам:\n` +
        `• 📊 Переглядати Вашу статистику балів\n` +
        `• 💰 Отримувати статистику та сповіщення про нові продажі\n\n` +
    ...
    this.bot.on('contact', async (msg) => {
      ...
      try {
        ...
        // Оскільки в БД номери можуть бути в форматі "+38 (0ХХ) ХХХ-ХХ-ХХ"
        // а ми шукаємо "+380ХХХХХХХХХ", використаємо findMany і порівняємо нормалізовані номери
        ...
        if (!user) {
          await this.bot.sendMessage(
            ...
            `Переконайтеся що ви зареєстровані як лікар у системі "Ортомат".`,
            {
              reply_markup: {
        ...
        if (user.role !== 'DOCTOR') {
          await this.bot.sendMessage(
            chatId,
            `❌ Цей сервіс доступний тільки для лікарів системи "Ортомат".\n\n` +
            `Ваша роль: ${user.role}`,
            {
              reply_markup: {
                remove_keyboard: true,
